...
class EtFileSystem:
    ...
    def read(cls, file_name: str):
        ...
        cls.__file.seek(260)
        cls.FILE_COUNT = struct.unpack("<I", cls.__file.read(4))[0]  # [0] because the return type is a tuple
        ...
        for f in range(cls.FILE_COUNT):
            ...
            file = EtFile(location=cls.__file.read(256).decode("utf-8", "ignore").split('\x00', 1)[0])
